...
class CustomActivityIndicator extends StatefulWidget {
  ...
  /// to 1.0.
  const CustomActivityIndicator.partiallyRevealed({
    Key? key,
    this.radius = _kDefaultIndicatorRadius,
    this.progress = 1.0,
  })  : assert(radius > 0.0),
  ...
  /// Typical usage would display all ticks, however, this allows for more
  /// fine-grained control such as during pull-to-refresh when the drag-down
  /// action shows one tick at a time as the user continues to drag down.
  ///
...
class _CustomActivityIndicatorState extends State<CustomActivityIndicator>
  ...
  @override
  void didUpdateWidget(CustomActivityIndicator oldWidget) {
...
/// Alpha values extracted from the native component
/// (for both dark and light mode) to draw the spinning ticks.
...
class _CustomActivityIndicatorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < tickCount * progress; ++i) {
      ...
      canvas
        ..drawRRect(tickFundamentalRRect, paint)
